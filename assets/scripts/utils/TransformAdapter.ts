...
import { _decorator, Component, Enum, UITransform, Size, Vec2, v2, View, view, Director, director, size, Node, UIOpacity, screen } from 'cc';
...
export class TransformAdapter extends Component {
	...
	@property
	public isInstant: boolean = false;
	@property({ visible(this: TransformAdapter) { return this.isInstant } })
	public hideWhileResize: boolean = false;
	...
	onTransformEvent() {
		...
		if (this.isInstant) {
			transformFunc();
			if (this.hideWhileResize) {
				this._uiOpacity.opacity = 255;
			}
		} else {
			this.scheduleOnce(transformFunc);
		}
	}
